...
CAMERASLACK_V = 90   # horizontal camera slack
CAMERASLACK_H = 180  # vertical camera slack
...
LEFT = 'left'
RIGHT = 'right'
# upwards and downwards direction bounce
UPWARDS = 1
DOWNWARDS = -1
# lost size
LOSTSIZE = 5
...
def runGame():
    ...
    while True:  # main game loop
        ...
        for sObj in squirrelObjs:
            sObj['rect'] = pygame.Rect((sObj['x'] - camerax,
                                        sObj['y'] - cameray - getBounceAmount(sObj['bounce'], sObj['bouncerate'],
                                                                              sObj['bounceheight'], sObj['bounceDir']),
                                        sObj['width'],
                                        sObj['height']))
        ...
        # disabled health system because we dont need it anymore
            ...
            for i in range(len(squirrelObjs)-1, -1, -1):
                ...
                if 'rect' in sqObj and playerObj['rect'].colliderect(sqObj['rect']):
                    ...
                    if sqObj['width'] * sqObj['height'] <= playerObj['size']**2:
                        ...
                        # player size gets smaller
                        playerObj['size'] -= int((sqObj['width'] * sqObj['height'])**0.2) + 1
                        # minimum player size is 0 so game doesnt crash if it goes below 0
                        if playerObj['size'] < 0:
                            playerObj['size'] = 0
                        # update player new size
                        if playerObj['facing'] == LEFT:
                            playerObj['surface'] = pygame.transform.scale(L_SQUIR_IMG, (playerObj['size'], playerObj['size']))
                        if playerObj['facing'] == RIGHT:
                            playerObj['surface'] = pygame.transform.scale(R_SQUIR_IMG, (playerObj['size'], playerObj['size']))
...
def getBounceAmount(currentBounce, bounceRate, bounceHeight, bounceDir=UPWARDS):
    # Returns the number of pixels to offset based on the bounce.
    # Larger bounceRate means a slower bounce.
    # Larger bounceHeight means a higher bounce.
    # currentBounce will always be less than bounceRate
    return int(math.sin((math.pi / float(bounceRate)) * currentBounce) * bounceHeight) * bounceDir
...
def makeNewSquirrel(camerax, cameray):
    ...
    # bounce direction
    if random.randint(0, 1) == 0:
        sq['bounceDir'] = UPWARDS
    else:
        sq['bounceDir'] = DOWNWARDS
    return sq
